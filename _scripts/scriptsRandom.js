  ...
  for(var row = 0; row < ROWS; row++)
  {
    for(var column = 0; column < COLUMNS; column++)
    {
      ...
      if(currentTile !== EMPTY)
      {
        ...
        console.log("Map Occupied Value: " + "Row: " + row + " Col: "+ column + " T/F: " + gameVar.grid[row][column].occupied);
      }
    }
  }
}
...
function createOtherObjects()
{
  ...
  do {
      gameVar.exitCOL = gameVar.randomBetween(0,ROWS);
      gameVar.exitROW = gameVar.randomBetween(0,COLUMNS);
  } while (checkTile(gameVar.exitCOL, gameVar.exitROW, FLOOR));
  gameVar.grid[gameVar.exitCOL][gameVar.exitROW].occupied = true;
  gameVar.exitKEY.x = gameVar.exitROW * SIZE;
  gameVar.exitKEY.y = gameVar.exitCOL * SIZE;
  ...
  // Game Player Object. Randomly placed
      ...
      } while (checkTile(gameVar.playerROW, gameVar.playerCOL, FLOOR));
      player.x = gameVar.playerCOL * SIZE;
      player.y = gameVar.playerROW * SIZE;
